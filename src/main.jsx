...
import {
  ...
  Sparkles, Sun, Trash2, UserRound, Users, X, Zap
} from 'lucide-react';
import './styles.css';
...
function App() {
  ...
  function loginChild(id) {
    const child = children.find((item) => item.id === id);
    setTokenChild(child || null);
    setTokenInput('');
    setShowToken(true);
  }
  ...
  async function saveAccessToken() {
    ...
    if (!token || !tokenChild) { notify('请粘贴有效的 Access Token', 'error'); return; }
    try {
      await apiRequest(`/api/children/${encodeURIComponent(tokenChild.id)}/login`, { method: 'POST', body: JSON.stringify({ accessToken: token }) });
      await refreshState(false);
      setShowToken(false);
      notify('AT 已保存到服务端并进入待加入池');
    } catch (error) { notify(`AT 保存失败：${error.message}`, 'error'); }
  }
  ...
  return <div className="app-shell">
    ...
    <main className={`main-content ${view === 'run' ? 'home-content' : ''}`}>
      ...
      {view === 'free' && <FreeAccountsView children={filteredAccounts} allChildren={accountRecords} mothers={mothers} search={search} setSearch={setSearch} setShowImport={() => setShowImport(true)} addAccount={() => openAccount()} exportSub2Api={exportSub2Api} pushSub2Api={pushSub2Api} removeChild={removeChild} loginChild={loginChild} editAccount={openAccount} acquireAccount={acquireAccount} acquireStates={acquireStates} />}
    ...
    {showToken && <Modal title="录入子号 AT" onClose={() => setShowToken(false)}><div className="modal-intro">账号：<strong>{tokenChild?.email}</strong><br />请先在浏览器完成登录、验证码和空间切换，再粘贴该账号的实际 Access Token。值只保存在当前浏览器。</div><input className="token-input" type="password" value={tokenInput} onChange={(event) => setTokenInput(event.target.value)} placeholder="粘贴 Access Token" /><div className="modal-foot"><span className="muted">不会自动向第三方提交凭据。</span><button className="button primary" onClick={saveAccessToken}><KeyRound size={15} />保存 AT</button></div></Modal>}
...
function FreeAccountsView({ children, allChildren, mothers, search, setSearch, setShowImport, addAccount, exportSub2Api, pushSub2Api, removeChild, loginChild, editAccount, acquireAccount, acquireStates }) {
...
function ChildrenView({ children, search, setSearch, setShowImport, exportSub2Api, removeChild, loginChild }) { return <section className="content-panel"><div className="content-toolbar"><div><h2>子号维护</h2><p>管理登录凭据、加入过的空间和可用额度。</p></div><div className="toolbar-actions"><button className="button ghost" onClick={exportSub2Api}><Download size={15} />导出 Sub2API</button><button className="button primary" onClick={setShowImport}><Plus size={15} />导入子号</button></div></div><div className="filter-row"><div className="search-box"><ListFilter size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索邮箱、账号或空间" /></div><span className="result-count">共 {children.length} 个账号</span><button className="button ghost"><ArrowDownToLine size={15} />批量操作</button></div><div className="table-wrap"><table className="data-table child-table"><thead><tr><th>账号</th><th>状态</th><th>5h 额度</th><th>7d 额度</th><th>当前空间</th><th>加入记录</th><th /></tr></thead><tbody>{children.map((child) => <tr key={child.id}><td><div className="account-cell"><div className="queue-avatar">{(child.email || '?')[0].toUpperCase()}</div><div><strong>{child.email || '未设置邮箱'}</strong><small className="mono">{child.id} · {child.plan || '未检测'}</small></div></div></td><td><StatusBadge status={child.status} /></td><td><QuotaBar value={child.quota5h} /></td><td><QuotaBar value={child.quota7d} /></td><td>{child.team || <span className="muted">待加入池</span>}</td><td>{child.joinedAt ? <span>{child.joinedAt}<small>{(child.workspaceHistory || []).length} 个空间</small></span> : <span className="muted">{child.hasAccessToken ? '待加入' : '待录入 AT'}</span>}</td><td><div className="row-actions"><button className="icon-button small" title="录入 AT" onClick={() => loginChild(child.id)} disabled={Boolean(child.hasAccessToken)}><KeyRound size={15} /></button><button className="icon-button small danger-hover" title="移出 Team" onClick={() => removeChild(child.id)}><Trash2 size={15} /></button></div></td></tr>)}</tbody></table></div></section>; }
...
function AccountModal({ account, onClose, onSave, onSaveAndAcquire, onAcquire, acquireState }) {
  const [form, setForm] = useState(() => ({ email: account?.email || '', password: '', totp: '', mailboxUrl: account?.mailboxUrl || '', verificationCode: '', callbackUrl: '' }));
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const busy = Boolean(acquireState?.loading);
  return <Modal title={account ? '编辑账号记录' : '新增 Free 账号'} onClose={onClose}>
    <div className="modal-intro">填写邮箱、密码和 2FA Secret 后保存账号，再点击“获取 Free JSON”。系统会自动登录并保存 AT、RT；若邮箱服务未配置，会在验证码处暂停等待。</div>
    <div className="form-grid"><label className="wide"><span>登录邮箱</span><input value={form.email} onChange={(event) => update('email', event.target.value)} placeholder="name@example.com" /></label><label><span>密码</span><input type="password" value={form.password} onChange={(event) => update('password', event.target.value)} placeholder="账号密码" /></label><label><span>2FA Secret</span><input type="password" value={form.totp} onChange={(event) => update('totp', event.target.value)} placeholder="可选，Base32 Secret" /></label><label className="wide"><span>邮箱 / 接码地址</span><input value={form.mailboxUrl} onChange={(event) => update('mailboxUrl', event.target.value)} placeholder="可选；支持 {email} 占位符" /></label><label><span>登录验证码</span><input inputMode="numeric" maxLength={6} value={form.verificationCode} onChange={(event) => update('verificationCode', event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="收到验证码后填写" /></label><label><span>浏览器回调地址</span><input value={form.callbackUrl} onChange={(event) => update('callbackUrl', event.target.value)} placeholder="验证后粘贴 localhost 回调 URL" /></label></div>
    <div className="account-acquire-panel"><div className="account-acquire-copy"><strong>Free JSON 获取</strong><span>已有 Sub2API refresh token 时直接刷新 AT；只有邮箱密码时会启动完整 OAuth 登录。</span><AcquireStatus state={acquireState} />{(acquireState?.authUrl || account?.login?.authUrl) && <a className="auth-link" href={acquireState?.authUrl || account?.login?.authUrl} target="_blank" rel="noreferrer">打开浏览器验证</a>}</div><div className="account-acquire-actions"><button className="button ghost compact-button" disabled={!account?.id || busy} onClick={() => onAcquire(account.id, 'free-json', form)}><CloudDownload size={14} />获取 Free JSON</button><button className="button secondary compact-button" disabled={!account?.id || busy} onClick={() => onAcquire(account.id, 'refresh-at', form)}><RefreshCw size={14} />刷新 AT</button></div></div>
    <div className="modal-foot"><span className="muted">当前账号：{account?.email || '未设置'}</span><div className="modal-foot-actions">{!account && <button className="button ghost" onClick={() => onSaveAndAcquire(form)}><CloudDownload size={14} />保存并获取 JSON</button>}<button className="button primary" onClick={() => onSave(form)}><Check size={15} />保存账号</button></div></div>
  </Modal>;
}
