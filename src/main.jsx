...
function App() {
  ...
  async function importAccountSub2Api(id, jsonText) {
    ...
    if (!json) { notify('请粘贴完整的 Sub2API JSON', 'error'); return; }
    try {
      const payload = await apiRequest(`/api/children/${encodeURIComponent(id)}/sub2api`, { method: 'PUT', body: JSON.stringify({ json }) });
      applyStatePayload(payload, setChildren, setMothers);
      await refreshState(false);
      setShowAccount(false);
      notify('Free Sub2API JSON 已录入');
    } catch (error) { notify(`Sub2API JSON 录入失败：${error.message}`, 'error'); }
  }
...
function FreeAccountsView({ children, allChildren, mothers, search, setSearch, setShowImport, addAccount, exportSub2Api, pushSub2Api, removeChild, deleteFreeAccount, loginChild, editAccount, acquireAccount, acquireStates }) {
...
function AccountModal({ account, onClose, onSave, onSaveAndAcquire, onAcquire, onImportSub2Api, acquireState }) {
  ...
  async function submitSub2ApiJson() {
    ...
    try { await onImportSub2Api(account.id, sub2apiJson); } finally { setImportingSub2apiJson(false); }
  }
